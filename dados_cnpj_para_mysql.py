...
resp = input(f'Isto irá CRIAR TABELAS ou REESCREVER TABELAS no database {dbname.upper()} no servidor Mysql {host} e MODIFICAR a pasta {pasta_saida}. Deseja prosseguir? (S/N)?')
if not resp or resp.upper()!='S':
    sys.exit()
#requer biblioteca pymysql, pip install pymysql
...
#%%
sqls = '''
ALTER TABLE empresas CHANGE COLUMN capital_social capital_social_str VARCHAR(20);
...
ALTER TABLE socios RENAME socios_original;
...
print('-'*20)
print('As tabelas foram criadas no servidor Mysql.')
